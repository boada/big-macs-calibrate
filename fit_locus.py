...
def get_survey_stars(inputcat,
    ...
    print 'WILL SEARCH FOR STARS WITHIN ' + str(RADIUS) + ' OF ' + str(
        RA) + ' ' + str(DEC)
    ...
    if survey == 'SDSS':
        ...
        keys = ['ra', 'dec']
        keys += [
            'psfMag_%(color)s - extinction_%(color)s + %(AB).2f as psfMagCorr_%(color)s'
            % {'color': color,
               'AB': AB} for color, AB in color_AB
        ]
        keys += ['psfMagErr_%(color)s' % {'color': color,
                                          'AB': AB} for color, AB in color_AB]
        keys += [
            '-2.5*LOG10(psfFlux_%(color)s) + 22.5 - extinction_%(color)s + %(AB).2f as psfPogCorr_%(color)s'
            % {'color': color,
               'AB': AB} for color, AB in color_AB
        ]
        keys += [
            '1.085/SQRT(psfFluxIvar_%(color)s)/psfFlux_%(color)s as psfPogErr_%(color)s'
            % {'color': color,
               'AB': AB} for color, AB in color_AB
        ]

        wherekeys = ['psfFlux_%(color)s > 0 ' % {'color': color,
                                                 'AB': AB}
                     for color, AB in color_AB]
        import sqlcl
        ...
        query = 'select ' + reduce(
            lambda x, y: x + ',' + y,
            keys) + ' from star as s JOIN dbo.fGetNearbyObjEq(' + str(
                RA) + ',' + str(DEC) + ',' + str(
                    RADIUS) + ' ) AS GN ON s.objID = GN.objID where s.clean=1 and ' + reduce(
                        lambda x, y: x + ' and ' + y, wherekeys)
        ...
        catalogStars = dict(zip(returned_keys, list([[]
                                                     for x in returned_keys])))
        ...
        command = "wget \"http://irsa.ipac.caltech.edu/cgi-bin/Gator/nph-query?outfmt=1&objstr=" + coordinate + "&spatial=Cone&radius=" + str(
            RADIUS) + "&radunits=arcmin&catalog=fp_psc&selcols=ph_qual,ra,dec,j_m,j_cmsig&constraints=ph_qual+like+%27A__%27+and+use_src%3D1\" -O " + catalog
        print command
        ...
        for line in lines:
            if line[0] == '|' and keyDict == {}:
                ...
                for key_full in returned_keys_full:
                    ...
                    keyDict[key_full.replace(' ', '')] = {'indexStart':
                                                          indexStart,
                                                          'indexEnd': indexEnd}
                ...
                for key in saveKeys:
                    value = float(line[keyDict[key]['indexStart']:keyDict[key][
                        'indexEnd']])
                    if key == 'j_m':
    ...
    if matched:
        ...
        if survey == 'SDSS' and sdssUnit:
            ...
            for column_name in returned_keys[2:]:
                cols.append(pyfits.Column(name=column_name,
                                          format='1E',
                                          array=scipy.array(catalogStars[
                                              column_name])))
            ...
            data_inputcat = zip(
                inputcat.data.field(racol), inputcat.data.field(deccol))
            ...
            for i in range(len(match)):
                if len(match[i]) == 1:
                    matchedStars += 1
                    for column_name in saveKeys:
                        hdu_new.data.field(column_name)[match[i][
                            0]] = catalogStars[column_name][i]
            ...
            if matchedStars > 3:
                ...
                print str(
                    matchedStars) + ' MATCHES WITH ' + survey + ' CATALOG'
                returnCat = inputcat
                matched = 0
...
def update_database(ebv,
    ...
    output = open('/Volumes/mosquitocoast/patrick/kpno/' + run + '/' + night +
                  '/' + snpath + '/slrfit', 'w')
    ...
    if prefix == 'SDSS':
        ...
        for key in zps_dict_all.keys():
            ...
            command = "UPDATE CALIB set " + "EBV=" + str(
                ebv) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)

            command = "UPDATE CALIB set " + "DUSTCORR=" + str(extinction_info[
                key]) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)

            command = "UPDATE CALIB set " + "GALLAT=" + str(
                gallat) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)

            command = "UPDATE CALIB set " + "SLRZP=" + str(zps_dict_all[
                key]) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)
            print results
            command = "UPDATE CALIB set " + "SLRNUM=" + str(results[
                'num']) + " WHERE SN='" + snpath + "' and FILT='" + key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)
            command = "UPDATE CALIB set " + "SLRREDCHI=" + str(results[
                'redchi']) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)
            command = "UPDATE CALIB set " + "SLRZPERR=" + str(results['errors'][
                key]) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)
            command = "UPDATE CALIB set " + "BOOTSTRAPNUM=" + str(results[
                'bootstrapnum']) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)
            command = "UPDATE CALIB set " + "BOOTSTRAPS='" + str(results['bootstraps'][
                key]) + "' WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
            c.execute(command)
    else:
        ...
        for key in zps_dict_all.keys():
            ...
            if key != 'JCAT':
                ...
                command = "UPDATE CALIB set " + prefix + "SLRZP=" + str(zps_dict_all[
                    key]) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
                c.execute(command)
                command = "UPDATE CALIB set " + prefix + "SLRNUM=" + str(results[
                    'num']) + " WHERE SN='" + snpath + "' and FILT='" + key + "' and NAME='" + name + "' and RUN='" + run + "'"
                c.execute(command)
                command = "UPDATE CALIB set " + prefix + "SLRREDCHI=" + str(results[
                    'redchi']) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
                c.execute(command)
                command = "UPDATE CALIB set " + prefix + "SLRZPERR=" + str(
                    results['errors'][
                        key]) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
                c.execute(command)
                command = "UPDATE CALIB set " + prefix + "BOOTSTRAPNUM=" + str(results[
                    'bootstrapnum']) + " WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
                c.execute(command)
                command = "UPDATE CALIB set " + prefix + "BOOTSTRAPS='" + str(
                    results['bootstraps'][
                        key]) + "' WHERE SN='" + snpath + "' and FILT='" + short_key + "' and NAME='" + name + "' and RUN='" + run + "'"
                c.execute(command)
...
def galactic_extinction_and_coordinates(RA, DEC):
    ...
    print 'RETRIEVING DUST EXTINCTION AT RA=' + str(RA) + ' DEC=' + str(
        DEC) + ' FROM NED'
    ...
    response = urllib.urlopen(
        'http://nedwww.ipac.caltech.edu/cgi-bin/nph-calc?' + reduce(
            lambda x, y: str(x) + '&' + str(y), form) + '"')
    text = response.read()
    ...
    for l in text.split('\n'):
        if found:
            ...
            gallat = float(res[1])
            break
        if string.find(l, 'Galactic') != -1 and string.find(l,
                                                            'Output:') != -1:
            found = True
...
def get_catalog_parameters(fulltable, racol, deccol):
    ...
    RA_DIFF_SQ = (
        (fulltable.data.field(racol) - RA) * 60. * scipy.cos(DEC))**2.
...
def run(file,
    ...
    necessary_columns = [racol, deccol] + [x['mag'] for x in input_info] + [x[
        'mag_err'] for x in input_info]
    ...
    if False:
        import pickle, pylab
        pylab.scatter(
            fulltable.data.field('psfPogCorr_z') - fulltable.data.field('j_m'),
        ...
        pylab.scatter(locus_list_mag['ZSDSS_JTMASS'][:],
                      locus_list_mag['ISDSS_ZSDSS'][:], color='green')
        pylab.show()
    ...
    offset_list = output_directory + '/' + file.split('/')[
        -1] + '.offsets.list'
    ...
    if not filter(lambda x: x.name == 'SeqNr', fulltable.columns):
        ...
        for col in fulltable.columns:
            cols.append(col)
        cols.append(pyfits.Column(name='SeqNr',
                                  format='J',
    ...
    def update_zps(zps_dict_all, zps_dict_all_err, cal_type, results,
                   red_or_blue):
    ...
    results, sdss_mags, SeqNr = fit(
        table,
        red_input_info,
        mag_locus,
        min_err=min_err,
        end_of_locus_reject=end_of_locus_reject,
        plot_iteration_increment=plot_iteration_increment,
        bootstrap=True,
        bootstrap_num=bootstrap_num,
        plotdir=plots_directory,
        pre_zps=None,
        live_plot=live_plot,
    ...
    if len(blue_input_info):
        ...
        for i in range(len(red_input_info)):
            ...
            if zps_dict_all.has_key(red_input_info[i]['mag']):
                red_input_info[i]['ZP'] = zps_dict_all[red_input_info[i][
                    'mag']]
                red_input_info[i]['ZPERR'] = zps_dict_all_err[red_input_info[
                    i]['mag']]
            else:
            ...
            print zps_dict_all
            print red_input_info[i]['mag'], red_input_info[i][
                'ZP']  #, zps_dict_all[red_input_info[i]['mag']]

        print red_input_info

        results, sdss_mags, SeqNr = fit(
            table,
            red_input_info + blue_input_info,
            mag_locus,
            min_err=min_err,
            end_of_locus_reject=end_of_locus_reject,
            plot_iteration_increment=plot_iteration_increment,
    ...
    if found2MASS:
        output_string += '#  USED ' + str(
            found2MASS) + ' MATCHED 2MASS STARS \n'

    output_string += '# ' + str(gallat) + ' ' + str(
        gallong) + ' galactic latitude longitude \n'
    ...
    for key in zps_dict_all.keys():
        ...
        output_string += key + ' ' + str(zps_dict_all[key]) + ' +- ' + str(
            zps_dict_all_err[key]) + ' ' + cal_type[key] + '\n'
    ...
    for filt_hold in info_hold:
        ...
        output_string += filt_hold['mag'] + ' ' + str(filt_hold[
            'ZP']) + ' +- -99 ' + cal_type[key] + '\n'
...
def fit(table,
    ...
    vary_input_info = filter(lambda x: x['HOLD_VARY'] == 'VARY',
                             input_info_unsorted)
    hold_input_info = filter(lambda x: x['HOLD_VARY'] == 'HOLD',
                             input_info_unsorted)
    ...
    for iteration in cycles:
        ...
        for i in range(len(hold_input_info)):
            ...
            if hold_input_info[i].has_key('ZPERR') and string.find(
        ...
        A_band = scipy.swapaxes(
            scipy.swapaxes(
                scipy.array(number_locus_points *
                            [[table.field(a['mag']) for a in input_info]]), 0,
                2), 1, 2)
        ...
        A_err = scipy.swapaxes(
            scipy.swapaxes(
                scipy.array(number_locus_points *
                            [[table.field(a['mag_err']) for a in input_info]]),
                0, 2), 1, 2)
        #print A_err.shape
        ...
        good[abs(A_band) == 0] = 0
        good = good[:, 0, :]
        good_bands_per_star = good.sum(
            axis=1)  # sum all of the good bands for any given star
        ...
        if string.find(iteration, 'bootstrap') != -1:
            ...
            locus_matrix = scipy.array(
                [locus_matrix[i] for i in random_indices])
        ...
        while keep_fitting:
            ...
            def errfunc(pars, residuals=False, savefig=None):
                ...
                ds_prelim = (bands - locus_matrix + zp_bands - mean_array
                             )**2.  #/ ds_err**2.
                ds_prelim[good == 0] = 0
                ''' calculate reduced chi squared '''
                ds = ds_prelim.sum(axis=2)**0.5
                resid_prelim = (bands - locus_matrix + zp_bands - mean_array
                                )**2. / bands_err**2.
                ...
                select_diff = resid[scipy.arange(len(match_locus_index)),
                                    match_locus_index]
                select_sum = resid_sum[scipy.arange(len(match_locus_index)),
                                       match_locus_index]

                #select_sum[match_locus_index > 30] = select_sum[match_locus_index > 30] / 20.

                select_good = good[scipy.arange(len(match_locus_index)),
                                   match_locus_index]

                dist = ds[scipy.arange(len(match_locus_index)),
                          match_locus_index]
                spectrum_normalization = mean[scipy.arange(len(
                    match_locus_index)), match_locus_index]
                ...
                norm = scipy.swapaxes(
                    scipy.array([spectrum_normalization.tolist()] * 5), 0, 1)
                sdss_locus_mags = sdss_locus_matrix[scipy.arange(len(
                    match_locus_index)), match_locus_index, :]
                ...
                print 'ZPs', dict(zip([a['mag'] for a in input_info], ([
                ...
                if live_plot and iteration is 'full' and (
                        itr % plot_iteration_increment == 0 or
                        savefig is not None):
                ...
                if residuals:
                    ...
                    end_of_locus = scipy.array(
                        [reduce(lambda x, y: x * y, [match_locus_index[
                            i] != x for x in range(end_of_locus_reject)])
                         for i in range(len(match_locus_index))])
            ...
            def plot_progress(pars, stat_tot=None, savefig=None):
                ...
                if pre_zps:
                    pre_zp_bands = scipy.swapaxes(
                        scipy.swapaxes(
                            scipy.array(number_locus_points * [
                                number_good_stars *
                                [[assign_zp(a[0], pars, pre_zps, zps_hold)
                                  for a in input_info]]
                            ]), 0, 1), 0, 0)
                    ...
                    for i in range(len(pre_zps)):
                        ...
                        zp_bands[:, :, i] = assign_zp(
                            a[0][0], pars, zps, zps_hold) - assign_zp(
                                a[1][0], pars, zps, zps_hold)

                oa = copy(input_info)
                oa.sort(sort_wavelength)

                oa_no_sdss = filter(
                    lambda x: string.find(x['mag'], 'psfMag') == -1, oa)
                ...
                for [c1_1, c1_2], [c2_1, c2_2] in index_list[:number_of_plots]:
                    ...
                    if ind(c1_band1) is not None and ind(
                            c1_band2) is not None and ind(
                                c2_band1) is not None and ind(
                                    c2_band2) is not None:
                        x_color = scipy.array(bands + zp_bands)[:, 0, ind(
                            c1_band1)] - scipy.array(
                                bands + zp_bands)[:, 0, ind(c1_band2)]

                        y_app_mag = scipy.array(bands + zp_bands)[:, 0, ind(
                            c2_band1)]
                        #print ind(c2_band1), ind(c2_band2)
                        ...
                        if pre_zps:
                            pre_x_color = scipy.array((
                                bands + pre_zp_bands)[:, 0,
                                                      color1_index].tolist())
                            pre_y_color = (
                                bands + pre_zp_bands)[:, 0, color2_index]
                        ...
                        if 'extinction' in input_info[ind(c1_band1)]:
                            x_extinct = input_info[ind(c1_band1)][
                                'extinction'] - input_info[ind(c1_band2)][
                                    'extinction']
                            y_extinct = input_info[ind(c2_band1)][
                                'extinction'] - input_info[ind(c2_band2)][
                                    'extinction']

                            gallat = '%.1f' % input_info[ind(c1_band1)][
                                'gallat']
                        else:
                        ...
                        if len(x_color):
                            pylab.scatter(x_color,
                                          y_color,
                                          color='#0066ff',
                                          s=4,
                                          marker='o',
                                          zorder=20)
                            pylab.errorbar(x_color,
                                           y_color,
                                           xerr=x_err,
                                           yerr=y_err,
                                           marker=None,
                            ...
                            if pre_zps:
                                pylab.errorbar(pre_x_color,
                                               pre_y_color,
                                               xerr=x_err,
                                               yerr=y_err,
                                               fmt=None,
                                               c='green')
                                pylab.scatter(pre_x_color,
                                              pre_y_color,
                                              c='green')
                            ...
                            if not publish:
                                pylab.text(
                                    c1_locus[0] + x_diff * 0.1 + x_extinct,
                                    c2_locus[-1] - y_diff * 0.1 + y_extinct,
                                    '  ext. vec.',
                                    color='black')
                            if stat_tot is not None and not publish:
                                pylab.title('N=' + str(len(x_color)) +
                                            ' chi$^{2}$=' + ('%.1f' % stat_tot)
                                            + ' ' + iteration + ' ' + outliers
                                            + ' GALLAT=' + str(gallat))
                            ...
                            fit_band_zps = reduce(lambda x, y: x + y, [
                                z[-2:].replace('C', '').replace('-', '')
                                for z in [a['mag'] for a in input_info]
                            ])
                            print 'savefig', savefig
                            ''' only save figure if savefig is not None '''
                            if savefig is not None:
                                if (string.find(iteration, 'bootstrap') == -1
                                        or save_bootstrap_plots):
                                    file = plotdir + '/qc_' + fit_band_zps + '_' + x_color_name.replace(
                                        units, '').replace(
                                            ' ',
                                            '') + '_' + y_color_name.replace(
                                                units, '').replace(
                                                    ' ',
                                                    '') + '_' + savefig.replace(
                                                        ' ', '_')
                ...
                def order_plots(a, b):
                    if string.find(a, 'psfMag') != -1 and string.find(
                            b, 'psfMag') == -1:
                        return 1
                    else:
                        return -1
                ...
                if savefig is not None:
                    ...
                    html.write('<html>\n')
                    for type in ['full_outliers_removed',
                                 'full_egregious_outliers_removed',
                                 'no_outlier_rejection']:
                        html.write('<h1>' + type + '</h1>\n')
                        for f in fs:
                            if string.find(f.split('/')[-1], type) != -1:
                                html.write('<img src=' + f.split('/')[-1] +
                                           '></img><br>\n')
                    html.close()
            ...
            if iteration == 'full':
                ...
                if True:
                    ...
                    for i in range(len(hold_input_info), len(input_info)):
                        ...
                        info_hold = filter(lambda x: x['HOLD_VARY'] == 'HOLD',
                                           input_info)  #[0]['mag']
                        ...
                        print scipy.sum(
                            good[:, 0,
                                 i]), 'number of good measurements in band'
                        ...
                        locus_here = [mag_locus[x][input_info[i]['mag']] -
                                      mag_locus[x][info_hold[0]['mag']]
            ...
            out = scipy.optimize.fmin(errfunc,
                                      pinit,
                                      maxiter=10000,
                                      maxfun=100000,
                                      ftol=0.00001,
            ...
            if iteration is 'full':
                errfunc(out,
                        savefig=(iteration + '_' + outliers + '.png').replace(
                            '$', ''))
            #print out
            ...
            if fit_num == 1:
            ...
            elif number_good_stars > len(locus_matrix) or len(filter(
                    lambda x: x is False, end_of_locus.tolist())) > 0:
                print 'REFITTING AFTER REMOVING ' + str(number_good_stars - len(
                    locus_matrix)) + ' OUTLIERS AND STARS MATCHING BLUE END OF LOCUS'
                number_good_stars = len(locus_matrix)
                ...
                if False:
                    pinit = scipy.array(out) + scipy.array(
                        [random.random() * 1.0 for p in pinit])
                    pinit = out
                    out = scipy.optimize.fmin(
                        errfunc, pinit, maxiter=10000,
                        args=())
                    residuals, dist, redchi, end_of_locus, num, sdss_mags = errfunc(
                        out,
                        savefig=iteration + '_' + outliers + '.png',
                        residuals=True)
                    print out
            else:
    ...
    for key in [a['mag'] for a in input_info]:
        ...
        if bootstrap_num > 0 and len(l) > 0:
            bootstraps[key] = reduce(lambda x, y: x + ',' + y,
                                     [str(z) for z in l])
        else:
            bootstraps[key] = 'None'

    results['hold_vary'] = dict(zip([a['mag'] for a in input_info], [a[
        'HOLD_VARY'] for a in input_info]))
    ...
    if False:
        ...
        def save_results(save_file, results, errors):
            f = open(save_file, 'w')
            for key in results['full'].keys():
                f.write(key + ' ' + str(results['full'][key]) + ' +- ' + str(
                    errors[key]) + '\n')
            f.close()
